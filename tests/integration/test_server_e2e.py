...
def test_reasoning_engine_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    has_content = any(
        ...
        and any(part.get("text") or part.get("inlineData") or part.get("inline_data") for part in event["content"]["parts"])
        for event in events
    )
    assert has_content, "No content (text or inline A2UI blob) in reasoning_engine events"
